...
class ConfigProviders(Config):
    ...
    def saveProviders(self, newznab_string='', torrentrss_string='', provider_order=None, **kwargs):
        ...
        for curProvider in sickbeard.providers.sortedProviderList():
            ...
            #for curlProvider in [prov for prov in sickbeard.providers.sortedProviderList() if
                                   #prov.provider_type == GenericProvider.DDL]:

            if hasattr(curProvider, 'storageProviderAllow'):
                try:
                    curProvider.storageProviderAllow['Uptobox'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_uptobox'])
                except Exception:
                    curProvider.storageProviderAllow['Uptobox'] = False

                try:
                    curProvider.storageProviderAllow['Uplea'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_uplea'])
                except Exception:
                    curProvider.storageProviderAllow['Uplea'] = False

                try:
                    curProvider.storageProviderAllow['1fichier'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_1fichier'])
                except Exception:
                    curProvider.storageProviderAllow['1fichier'] = False

                try:
                    curProvider.storageProviderAllow['Uploaded'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_uploaded'])
                except Exception:
                    curProvider.storageProviderAllow['Uploaded'] = False

                try:
                    curProvider.storageProviderAllow['Rapidgator'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_rapidgator'])
                except Exception:
                    curProvider.storageProviderAllow['Rapidgator'] = False

                try:
                    curProvider.storageProviderAllow['TurboBit'] = config.checkbox_to_value(
                        kwargs[curProvider.get_id() + '_allow_turboBit'])
                except Exception:
                    curProvider.storageProviderAllow['TurboBit'] = False

                try:
                    curProvider.storageProviderAllow['Mega'] = config.checkbox_to_value(
